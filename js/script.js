...
$(document).ready(function(){
  ...
  $('#btn-search').click(function(){
    ...
    var wikiApiUrl = 'https://en.wikipedia.org/w/api.php?action=opensearch&search=' + query + '&format=json&callback=?';
    ...
    var wikiRequestTimeout = setTimeout(function(){
      $wikiArticles.text('Sorry, the Wikipedia articles cannot be loaded as your request timed out.');
    }, 8000);
    ...
    .done(function(data) {
      ...
      if(wikiArticlesLength == 0){
        $wikiArticles.append('Sorry, <b>' + query + '</b> returned no results.');
      }
      else {
        for(var i = 0; i < wikiArticlesLength; i++) {
          ...
          $('<li><h3><a href="' + articleUrl + '" target="blank">' +
                                  articleTitle + '</a></h3><p class="url">' +
                                  articleUrl + '</p><p>' +
                                  articleSnippet + '</p></li>').appendTo($wikiArticles);
        }
        $('<li><h3><a href="https://en.wikipedia.org/w/index.php?title=Special:Search&profile=default&fulltext=1&search=' + query + '" target="blank">See all search results for <b>' + query + '</b> on Wikipedia...</a></h3></li>').appendTo($wikiArticles);
      }
      clearTimeout(wikiRequestTimeout);
    })
    .fail(function(errorMsg) {
      $wikiArticles.text('Sorry, the Wikipedia articles cannot be loaded as the request failed.');
